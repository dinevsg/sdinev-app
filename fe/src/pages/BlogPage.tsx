  ...
  const navigate = useNavigate();
   ...
   useEffect(() => {
    const fetchBlogPosts = async () => {
      try {
        const res = await fetch(`${apiUrl}/blog/latest/?format=json`);
...
//   if (loading) return <p className="text-center py-12">Loading blog posts...</p>;
//   if (error) return <p className="text-center py-12 text-red-500">{error}</p>;
//   if (!blogPosts.length) return <p className="text-center py-12">No blog posts available.</p>;
...
//               </div>
//               <h3
//                 className="text-xl lg:text-2xl font-bold text-neutral-main cursor-pointer"
//                 onClick={() =>
//                   handleNavigate(`/blog/${featuredPost.category_slug}/${featuredPost.slug}`)
//                 }
//               >
...
//                     src={post.picture}
//                     alt={post.title}
//                     className="border border-gray-700 w-full h-48 object-cover rounded-2xl transition-all duration-200 group-hover:scale-[1.02] shadow-lg group-hover:shadow-gray-700 group-hover:shadow-md cursor-pointer"
//                     onClick={() =>
//                       handleNavigate(`/blog/${post.category_slug}/${post.slug}`)
//                     }
//                   />
