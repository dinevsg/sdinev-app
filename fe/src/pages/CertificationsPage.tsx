        ...
        <div className="hidden md:block">
          <TabsProvider defaultValue="all">
            <div className="flex justify-center mb-4">
              <div className="flex w-full items-center xl:w-fit p-2 xl:p-1 justify-between text-neutral-main border bg-slate-800/20 border-gray-700 rounded-2xl">
                <TabsBtn value="all">
                  <span className="relative z-[2] font-medium">All</span>
                </TabsBtn>
                {allProviders.map(provider => (
                  <TabsBtn key={provider} value={provider.toLowerCase()}>
                    <span className="relative z-[2] font-medium">{provider}</span>
                  </TabsBtn>
                ))}
              </div>
            </div>
            ...
            {allProviders.map(provider => (
              <TabsContent key={provider} value={provider.toLowerCase()} className="w-full">
                <div className="min-h-[520px] flex flex-col items-center justify-center">
                  {certifications.filter(cert => cert.provider.toLowerCase() === provider.toLowerCase()).length > 0 ? (
                    <div className="grid w-full auto-rows-fr grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 items-stretch justify-items-center gap-4">
                      {certifications
                        .filter(cert => cert.provider.toLowerCase() === provider.toLowerCase())
                        .map((cert, index) => (
                          <CertificationCard key={cert.id} cert={cert} index={index} />
                        ))}
                    </div>
